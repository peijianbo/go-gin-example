...
func RecoveryWithWriter(out io.Writer) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		defer func() {
			if r := recover(); r != nil {
				code := e.ERROR
				errorMsg := fmt.Sprintf("%s", r)
				httpCode := http.StatusInternalServerError
				if err, ok := r.(MyError); ok {
					errorMsg = e.GetMsg(err.Code)
					if extra := err.Extra; extra != "" {
						errorMsg = fmt.Sprintf("%s(%s)", errorMsg, extra)
					}
				}
				stack := stack(3)
				logger.Printf("%s\n%s", errorMsg, stack)
				c.AbortWithStatusJSON(httpCode, app.Response{Code: code, Msg: errorMsg, Data: ""})
			}
		}()
		c.Next()
	}
